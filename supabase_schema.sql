...
-- 1. Atualizar Status de Agendamento (Adicionar pending_approval, expired, rejected, cancelling)
DO $$
BEGIN
    ALTER TABLE public.appointments DROP CONSTRAINT IF EXISTS appointments_status_check;
    ALTER TABLE public.appointments ADD CONSTRAINT appointments_status_check
        CHECK (status IN ('pending', 'scheduled', 'confirmed', 'in_progress', 'completed', 'cancelled', 'blocked', 'reserved', 'failed', 'pending_approval', 'expired', 'rejected', 'no_show', 'awaiting_payment', 'cancelling'));
EXCEPTION
    WHEN others THEN NULL;
END $$;
...
create table if not exists public.notifications (
  ...
  type text not null check (type in ('booking_request', 'booking_accepted', 'booking_rejected', 'booking_cancelled', 'booking_expired', 'payment_released', 'reminder', 'system', 'tip')),
...
-- ==============================================================================
-- ADDITIVE FINANCIAL ARCHITECTURE: ASAAS INSTALLMENTS AND CASH FLOW
-- ==============================================================================
...
CREATE INDEX IF NOT EXISTS idx_payment_installments_provider_payment_id ON public.payment_installments (provider_payment_id);
CREATE INDEX IF NOT EXISTS idx_payment_installments_group_id ON public.payment_installments (group_id);
CREATE INDEX IF NOT EXISTS idx_payment_installments_instructor_status ON public.payment_installments (instructor_id, status);
CREATE INDEX IF NOT EXISTS idx_payment_installments_due_date ON public.payment_installments (due_date);
CREATE INDEX IF NOT EXISTS idx_payment_installments_appointment_id ON public.payment_installments (appointment_id);
