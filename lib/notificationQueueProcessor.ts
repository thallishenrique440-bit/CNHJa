...
// Supported execution modes for the worker
...
// All known domain states of a notification job
...
// Generate an immutable unique instance ID upon startup
...
/**
 * Standardized, structured logger for notification queue operations.
 */
export function logWorker(event: LogEvent, cycleId?: number, payload?: Record<string, unknown>) {
  ...
  console.log(`[NotificationProcessor] ${JSON.stringify(logObj)}`);
}
...
export async function executeWorkerCycle() {
  ...
  if (!init) return { success: false, reason: 'init_failed', processed: 0 };
  ...
  if (isCycleRunning) {
    ...
    return { success: false, reason: 'overlap_prevented', processed: 0 };
  }
  ...
  try {
    ...
    if (workerMode === 'active') {
      ...
      if (error) {
        ...
        console.log(`[NotificationProcessor] Processando fila (${claimedJobs.length} jobs)`);
        ...
        console.log('[NotificationProcessor] Aguardando novos jobs');
    ...
    return { success: true, claimed: claimedCount, processed: processedJobs };
  } catch (err: unknown) {
    const errMsg = err instanceof Error ? err.message : String(err);
    logWorker('POLLING_ERROR', currentCycleId, { error: errMsg });
    return { success: false, error: errMsg, processed: processedJobs };
  } finally {
