...
export class InstructorProjector {
  ...
  public static async project(
    ...
    // 1. Projection with Optimistic Locking Retry Loop (up to 3 attempts)
    const MAX_RETRIES = 3;
    let attempt = 0;
    ...
    while (attempt < MAX_RETRIES) {
      ...
      try {
        ...
        if (
          ...
          payload.eventType === ProjectionSourceEventType.CANCELLATION_CREATED
        ) {
          record.future_receivables = Math.max(0, record.future_receivables - net);
        } else if (payload.eventType === ProjectionSourceEventType.STATE_TRANSITION) {
          const status = (payload.status || 'PENDING').toUpperCase();

          if (status === 'PENDING' || status === 'AUTHORIZED') {
            if (!payload.eventId || !payload.eventId.startsWith('sched_')) {
              record.future_receivables += net;
            }
        ...
        if (err.name === 'ProjectionOptimisticLockError' && attempt < MAX_RETRIES) {
          ProjectionLogger.warn(
            'InstructorProjector',
            `Optimistic locking conflict on attempt ${attempt}, retrying...`,
            { identifier: instructorId }
          );
          continue;
        }
    ...
    return { outcome: ProjectionOutcome.ERROR, error: 'Max retries exceeded' };
