-- =============================================================================
-- P-1.20.5 FASE F (2/2) — CORRECAO DA NOTIFICACAO DA RPC P-1.20.4
--
-- A migration historica `20260923_p1204_01_reschedule_appointment_direct.sql`
-- JA FOI APLICADA EM PRODUCAO e NAO e' editada. Esta migration a corrige por
-- CREATE OR REPLACE, com a MESMA assinatura
-- (uuid[], date, time) — o que preserva a ACL concedida anteriormente.
--
-- DEFEITO CORRIGIDO (confirmado em producao):
--   appointment 5d71e28d-3bdf-4d98-af40-4bf3599051c2, remarcado as 15:00:43Z,
--   group_id 0f06c067-3c1d-4bba-9640-0895501df16c.
--   A notificacao 423926e9 ('Nova solicitacao de aula', type='booking_request',
--   mesmo user_id de instrutor, mesmo group_id) ja existia desde as 14:58:17.
--   O INSERT da remarcacao violou idx_notifications_idempotency_group,
--   create_unified_notification engoliu o unique_violation e devolveu o id
--   antigo. Resultado medido: 0 notificacoes e 0 notification_jobs apos 15:00,
--   com 2 tokens FCM validos para o instrutor. A cadeia FCM estava sadia.
--
-- ESCOPO: SOMENTE o bloco de notificacao.
--   - assinatura: identica;
--   - regra de negocio: identica;
--   - validacoes: identicas (0/1..13 inalteradas);
--   - colunas escritas: identicas (date, start_time, end_time, rescheduled_at,
--     updated_at);
--   - neutralidade financeira: identica.
-- Diferencas em relacao a versao em producao, exaustivamente:
--   1. p_type  : 'booking_request' -> 'reschedule_applied';
--   2. p_group_id : v_group_id -> NULL.
--
-- DEPENDE DE: 20260923_p1205_02_reschedule_notification_types.sql
-- (o tipo 'reschedule_applied' precisa existir em notifications_type_check
--  ANTES desta migration; caso contrario o INSERT viola o CHECK, a excecao cai
--  no bloco EXCEPTION WHEN OTHERS e vira apenas um WARNING silencioso).
--
