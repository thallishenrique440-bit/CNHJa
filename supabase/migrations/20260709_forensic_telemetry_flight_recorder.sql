-- Migration: Forensic Telemetry Flight Recorder for notify_new_notification()
-- Author: Forensic Audit Expert / AI Coding Agent
-- Date: 2026-07-09
-- Purpose: Diagnostic instrument only. Does not alter logic, behavior, or decisions.

-- 1. Create a persistent, secure audit table for execution tracking
CREATE TABLE IF NOT EXISTS public.audit_notify_new_notification (
...
-- 2. Recreate notify_new_notification with deep telemetry instrumentation (Layer 1 + Layer 2)
...
BEGIN
  ...
  SELECT COUNT(*) INTO v_key_count FROM public.notification_config WHERE key = 'service_role_key';
  ...
  IF v_edge_function_url IS NULL OR v_edge_function_url = '' THEN
    -- ==========================================
    -- [CP-05] AFTER_IF_URL_TRUE: Aborting due to invalid/missing URL
    -- ==========================================
  ...
  ELSE
    -- ==========================================
    -- [CP-06] AFTER_IF_URL_FALSE: URL is valid
    -- ==========================================
  ...
  IF v_service_role_key IS NULL OR v_service_role_key = '' THEN
    -- ==========================================
    -- [CP-08] AFTER_IF_KEY_TRUE: Aborting due to invalid/missing key
    -- ==========================================
  ...
  ELSE
    -- ==========================================
    -- [CP-09] AFTER_IF_KEY_FALSE: Key is valid
    -- ==========================================
  ...
  BEGIN
    ...
    VALUES (
      ...
      jsonb_build_object(
        'url', v_final_url, 
        'headers_safe', jsonb_build_object('Content-Type', 'application/json', 'Authorization', 'Bearer ' || v_key_status), 
