-- =============================================================================
-- P-1.20.5 — COMPATIBILIDADE ENTRE A RPC P-1.20.4 E O MODELO DE PROPOSTA
--
-- MOTIVO (defeito encontrado por teste, nao por suposicao):
--   o teste 57 da bateria P-1.20.5 em PostgreSQL 16 efemero reprovou.
--   `reschedule_appointment_direct` bloqueia remarcacao direta apenas quando
--   `reschedule_requested_at IS NOT NULL`. O modelo de proposta introduzido em
--   20260923_p1205_01 grava o estado em `proposal_status='pending'` e NAO
--   escreve `reschedule_requested_at` (coluna legada, preservada intacta por
--   decisao explicita da FASE B).
--   Resultado sem esta migration: com uma proposta PENDENTE na aula, o aluno
--   >24h ainda conseguia mover a aula diretamente; em seguida um
--   accept_reschedule aplicaria o horario proposto sobre uma aula que ja havia
--   mudado de lugar.
--
-- ESCOPO: UMA unica condicao de guarda. Exaustivamente:
--   count(*) FILTER (WHERE a.reschedule_requested_at IS NOT NULL)
--     -> count(*) FILTER (WHERE a.reschedule_requested_at IS NOT NULL
--                            OR a.proposal_status = 'pending')
--   O codigo de erro devolvido continua sendo RESCHEDULE_PENDING.
--   Assinatura, colunas escritas, grade, regra das 24h, conflitos e
--   neutralidade financeira: inalterados.
--
-- Esta migration e' apresentada SEPARADA de
-- 20260923_p1205_03_fix_reschedule_direct_notification.sql precisamente para
-- que a correcao de notificacao continue sendo, sozinha, "somente notificacao".
--
-- DEPENDE DE: 20260923_p1205_01, _02 e _03 (aplicar nesta ordem).
--
...
BEGIN
  ...
  --    P-1.20.5: passa a considerar TAMBEM proposal_status='pending'. Sem isto,
  --    o aluno poderia mover a aula por baixo de uma proposta que a contraparte
  --    ainda vai responder — e o accept_reschedule aplicaria, depois, um horario
  --    proposto sobre uma aula que ja nao esta mais onde estava.
  ...
  BEGIN
    PERFORM public.create_unified_notification(
      ...
      -- P-1.20.5: tipo proprio de remarcacao. Antes ia como 'booking_request'.
      'reschedule_applied',
      CASE WHEN v_total > 1 THEN 'package' ELSE 'lesson' END,
      'instructor_agenda',
      v_total,
      -- P-1.20.5: p_group_id VOLTA A SER NULL.
      -- Passar v_group_id colidia com idx_notifications_idempotency_group
      -- (type, user_id, group_id), porque a notificacao `booking_request` da
      -- COMPRA ja ocupava essa chave. create_unified_notification captura
      -- unique_violation internamente e devolve o id existente SEM INSERIR,
      -- entao nenhuma excecao chegava ao bloco EXCEPTION abaixo e a RPC
      -- retornava 'ok' com zero notificacoes criadas.
      NULL,
      v_ids[1]
    );
